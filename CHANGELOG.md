...
- `--tls-client-cert` and `--tls-client-key` can now be used to configure a single TLS client certificate for use by `dbcrossbar`. Right now, this is only honored by the `postgres` driver and other drivers that build upon it.
