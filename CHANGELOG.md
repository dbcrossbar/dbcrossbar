...
- The `csv:` driver has been superceded by a more generic `file:` driver. The old CSV driver **will continue to work, perhaps indefinitely,** but it will be increasingly hidden. By default, the `file:` driver determines the file format using the `--from-format` command line argument, or the file extension, falling back to CSV format as a last resort. If you encounter breaking changes, try adding `--from-format=csv` to your command line.
